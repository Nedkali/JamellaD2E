...
{
	MessageBoxA(hWnd, error, PROGRAMNAME, MB_OK | MB_ICONHAND | MB_APPLMODAL);
}
#if DEVELOPMENT == 1
...
{
	HKEY regkey;
	if (RegOpenKeyExA(HKEY_CURRENT_USER, "Software\\Blizzard Entertainment\\Diablo II", 0, KEY_READ, &regkey) == ERROR_SUCCESS)
	{
		DWORD type = REG_SZ;
		DWORD fdirsize = 260;
		DWORD x = RegQueryValueExA(regkey, "Save Path", 0, &type, (unsigned char*)dest, &fdirsize);
		if (x != ERROR_SUCCESS)
			return false;
		RegCloseKey(regkey);
		return true;
