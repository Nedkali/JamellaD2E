...
bool fileclass::loadfile(HWND hWnd, const char* tempfilename)
{
	...
	// Open File
	HANDLE hFile = CreateFileA(tempfilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

	if (hFile == INVALID_HANDLE_VALUE)
	{
		MessageBoxA(hWnd, "Could not open file!", PROGRAMNAME,	MB_OK | MB_ICONSTOP | MB_APPLMODAL);
		return false;
	}
	...
	if (filesize != fileread)
	{
		delete filedata;
		CloseHandle(hFile);
		MessageBoxA(hWnd, "Could not read from file!", PROGRAMNAME,	MB_OK | MB_ICONSTOP | MB_APPLMODAL);
		return false;
	}
	...
	if (erroffset >= 0)
	{
		...
		sprintf_s(text, "Corrupt field encountered in save game file! (@ offset %i)", erroffset);
...
inline bool check_written(HWND hWnd, DWORD a, DWORD b, HANDLE hFile)
{
	if (a != b)
	{
		sprintf_s(buffer, "Could not write to file at offset %i!", offset);
		MessageBox(hWnd, buffer, PROGRAMNAME, MB_OK | MB_ICONSTOP | MB_APPLMODAL);
		CloseHandle(hFile);
		return true;
	}
	else return false;
...
bool fileclass::savefile(HWND hWnd)
{
	...
	{ // Copy saved game file
		...
		{
			MessageBox(hWnd, "Could not make backup copy!\nStopping save.", PROGRAMNAME,MB_OK | MB_ICONEXCLAMATION | MB_APPLMODAL);
			return false;
		}
	} // Copy saved game file
	...
	if (hFile == INVALID_HANDLE_VALUE)
	{
		MessageBox(hWnd, "Could not create file!", PROGRAMNAME,	MB_OK | MB_ICONSTOP | MB_APPLMODAL);
		return false;
	}
	offset = 0;
	DWORD written;
		...
		for (Item* i = Items; i != 0; i = i->Next())
		{
			...
			if (i->Socketed())
			{
				...
				for (int g = 0; g < i->GemNum(); g++)
				{
					if (!G)
					{
						MessageBox(hWnd, "FATAL Gemmed Item Error!", PROGRAMNAME, MB_OK | MB_ICONSTOP | MB_APPLMODAL);
						return false;
					}
...
void fileclass::setfilename(const char* file)
{
	...
	{
		sprintf_s(filename, "%s\\%s.d2s", fdir, file);
	}
}
