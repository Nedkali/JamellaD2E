...
int PASCAL WinMain(HINSTANCE hInstance,	HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	HINSTANCE hRichEdit = LoadLibraryA("RICHED32.DLL");
	if (!hRichEdit)
	{
...
{
	char str[260];
	FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0,	(LPTSTR)str, 260, NULL);
	MessageBoxA(NULL, str, "Diablo 2 Save Game Editor",	MB_OK | MB_ICONINFORMATION);
	return false;
}
inline void DoCRC()
{
	// Open File
	HANDLE hFile = CreateFileA(ProgramFilePath(), GENERIC_READ, FILE_SHARE_READ, NULL,	OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

	if (hFile == INVALID_HANDLE_VALUE)
	{
		MessageBoxA(NULL, "Could not open program file for CRC checking!", PROGRAMNAME, MB_OK | MB_ICONSTOP | MB_APPLMODAL);
		exit(0);
	}
	...
	if (CRC != 0x00000000)
	{
		MessageBoxA(NULL, "CRC check of program file failed! This executable was tampered with.\nGet a new one from http:// jamella.dyns.cx!",PROGRAMNAME,	MB_OK | MB_ICONSTOP | MB_APPLMODAL);
		exit(0);
	}
	CloseHandle(hFile);
}
